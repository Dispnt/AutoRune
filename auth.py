...
game_path = 'E:\LeagueofLegends\lockfile'
try:
    server_info = open(game_path, "r").read().split(':')
except IOError:
...
summoner_info = requests.get(server_url+"/lol-summoner/v1/current-summoner", auth=HTTPBasicAuth('riot', server_pwd), verify=False).json()
